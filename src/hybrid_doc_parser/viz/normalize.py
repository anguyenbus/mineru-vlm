...
def doc_from_parser_output(
    ...
    for el in parser_output.elements:
        page = el.page_idx
        if backend in ("mineru", "paddleocr", "mineru25pro", "unlimited"):
            # Per-mille is page-relative; page size is not needed. PaddleOCR,
            # MinerU2.5-Pro and Unlimited-OCR pre-normalise their boxes to
            # per-mille at parse time.
